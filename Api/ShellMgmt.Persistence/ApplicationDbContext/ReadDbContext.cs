...
public class ReadDbContext(DbContextOptions<ReadDbContext> options) : DbContext(options)
{
    ...
    {
        throw new InvalidOperationException("ReadDbContext is read-only. SaveChanges() is not allowed.");
    }

    public override Task<int> SaveChangesAsync(CancellationToken cancellationToken = default)
    {
        throw new InvalidOperationException("ReadDbContext is read-only. SaveChangesAsync() is not allowed.");
    }
}
